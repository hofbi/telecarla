...
GstServerEncoder::GstServerEncoder(std::string name, PadProbeCallback encoderProbeCallback) noexcept
    : name_(std::move(name)), encoderProbeCallback_(std::make_unique<PadProbeCallback>(std::move(encoderProbeCallback)))
{
}

void GstServerEncoder::configureEncoderElement(std::unique_ptr<GstElement> encoderElement) noexcept
{
    ...
    auto wrappedProbeCallback = [](GstPad* pad, GstPadProbeInfo* info, void* userdata) {
        auto& callback = *static_cast<GstServerEncoder::PadProbeCallback*>(userdata);
...
const std::string& GstServerEncoder::getName() const noexcept
{
    return name_;
}

void GstServerEncoder::updateRateControlParameter(int bitrate)
{
